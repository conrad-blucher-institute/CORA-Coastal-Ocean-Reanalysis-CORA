...
def Calc_Expected_Diff(HL_Sub, HL_Con):
    ...
    if NLowsAbove ==0:
        SDC_Print(['Error. No Lows above mean.'])
    else:
        MeanLDiffAbove = MeanLDiffAbove / NLowsAbove

    if NHighsBelow == 0:
        SDC_Print(['Error. No Highs below mean.'])
    else:
        MeanHDiffBelow = MeanHDiffBelow / NHighsBelow

    if NLowsBelow == 0:
        SDC_Print(['Error. No Lows below mean.'])
    else:
...
def Local_Max_Fit(dt, wl, h):
    ...
    for l in range(first_in_window, last_in_window):
...
def Local_Min_Fit(dt, wl, l):
    ...
    for ll in range(first_in_window, last_in_window):
