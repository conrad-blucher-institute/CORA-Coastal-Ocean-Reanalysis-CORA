...
import os # JR added
from pathlib import Path # JR added
...
#################################################################### ###############################
#################################################################### ###############################
CONFIG_FILE = r"configpath\config.cfg" 
...
# end_of_path = fname.rfind('/') # JR commented out
# if end_of_path > -1: # JR commented out
#     path = fname[0:end_of_path+1] # JR commented out
# else: # JR commented out
#     path = '' # JR commented out

#Open the output file
# end_of_path = fname.rfind('1') # JR added, then commented out
head_tail= os.path.split(fname) # JR added
# path = head_tail[0] + '\\' # JR added
path = head_tail[0] + '\\Outputs\\' # JR added
file_stem = Path(head_tail[1]).stem # JR added
OutFile = open(path + file_stem + '.out', 'w') # JR added
...
#SDC_Print(['Using ', fname[end_of_path+1:]]) # JR commented out
SDC_Print(['Using ', head_tail[1]]) # JR added
f = open(fname, 'r')
...
f = open(path + file_stem + 'High-Lows.csv', 'w')
li=0
hi=0
